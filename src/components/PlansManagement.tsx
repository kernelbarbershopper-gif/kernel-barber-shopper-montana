                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-[#2A2A2A] flex items-center justify-center">
                          <Crown className={cn("w-5 h-5", plan.price === 0 ? "text-green-500" : "text-[#C9A84C]")} />
                        </div>
                        <div>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => openEdit(plan)}
                          className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-[#C9A84C]/10 text-[#C9A84C] hover:bg-[#C9A84C]/20 transition-all">
          ...
          <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl w-full max-w-lg p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.id} onChange={(e) => setForm((p) => ({ ...p, id: e.target.value }))}
                    ...
                    className="w-full bg-[#141414] border border-[#2A2A2A] rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#555] focus:outline-none focus:border-[#C9A84C] disabled:opacity-50" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#888] uppercase tracking-widest mb-1">Nome</label>
                  <input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))}
                    placeholder="Ex: Premium" className="w-full bg-[#141414] border border-[#2A2A2A] rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#555] focus:outline-none focus:border-[#C9A84C]" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  <input value={featureInput} onChange={(e) => setFeatureInput(e.target.value)}
                    ...
                    placeholder="Ex: Agendamento ilimitado" className="flex-1 bg-[#141414] border border-[#2A2A2A] rounded-xl px-4 py-2.5 text-sm text-white placeholder-[#555] focus:outline-none focus:border-[#C9A84C]" />
                ...
                <div className="flex flex-wrap gap-2">
                  {form.features?.map((f, idx) => (
                    <span key={idx} className="inline-flex items-center gap-1 text-[10px] bg-[#C9A84C]/10 text-[#C9A84C] px-2 py-1 rounded-lg">
                      {f}
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleSave} disabled={saving || !form.id || !form.name}
                className="flex-1 py-3 rounded-xl text-sm font-bold bg-gradient-to-r from-[#C9A84C] to-[#E8C96A] text-[#0A0A0A] hover:brightness-110 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
