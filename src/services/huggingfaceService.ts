...
export async function chatWithAI(message: string, context: string, history: { role: string; text: string }[] = []) {
  ...
  try {
    ...
    const systemPrompt = `Você é a **IA oficial do KERNEL BARBER SHOPPER** — uma plataforma SaaS completa para gestão de salãos. Seu PAPEL PRINCIPAL é apresentar, explicar e dar suporte sobre a própria plataforma.
...
KERNEL BARBER SHOPPER é um **SaaS de gestão para salãos**.
...
- **Pro:** salãos em crescimento
- **Enterprise:** Redes e grandes operações

## SUA FUNÇÃO PRINCIPAL:
- **Apresentar a plataforma** — explique o que é o KERNEL BARBER SHOPPER, seus módulos, benefícios e planos
