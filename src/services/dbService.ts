...
export const subscribeToPlans = (callback: (data: any[]) => void) => {
  ...
  const channel = supabase.channel('plans-changes-' + Date.now());
...
export const subscribeToCollection = <T>(
  ...
  const channelName = `collection-${path}-${cleanShopId}-${Date.now()}`;
  const channel = supabase.channel(channelName);

  channel.on('postgres_changes',
    { event: '*', schema: 'public', table: path, filter: `shop_id=eq.${cleanShopId}` },
    () => fetchWithRetry(() => fetchData()).catch(e => console.error('fetchData error:', e))
  );
...
export const subscribeToShops = (callback: (data: any[]) => void) => {
  ...
  const channel = supabase.channel('shops-changes-' + Date.now());
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'shops' }, () => fetchWithRetry(() => fetchShops()).catch(e => console.error('fetchShops error:', e))).subscribe();
...
export const subscribeToSubscriptions = (callback: (data: any[]) => void) => {
  ...
  fetch();
  const channel = supabase.channel('subs-changes-' + Date.now());
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'subscriptions' }, () => fetch()).subscribe();
  return () => supabase.removeChannel(channel);
};
...
export const subscribeToPayments = (callback: (data: any[]) => void) => {
  ...
  fetch();
  const channel = supabase.channel('pays-changes-' + Date.now());
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'payments' }, () => fetch()).subscribe();
  return () => supabase.removeChannel(channel);
};
...
export const subscribeToWithdrawals = (callback: (data: any[]) => void) => {
  ...
  fetch();
  const channel = supabase.channel('wd-changes-' + Date.now());
  channel.on('postgres_changes', { event: '*', schema: 'public', table: 'withdrawals' }, () => fetch()).subscribe();
  return () => supabase.removeChannel(channel);
};
...
export const subscribeToMachineRequests = (shopId: string, callback: (data: any[]) => void) => {
  const fetchData = async () => {
    const { data, error } = await supabase.from('machine_requests').select('*').eq('shop_id', shopId).limit(1).maybeSingle();
    if (!error && data) callback([data]);
  };
  fetchData();
  const channel = supabase.channel(`machine_requests:${shopId}`);
  channel.on('postgres_changes' as any, { event: '*', schema: 'public', table: 'machine_requests', filter: `shop_id=eq.${shopId}` }, (payload: any) => {
    fetchData();
  }).subscribe();
  return () => { supabase.removeChannel(channel); };
};

export const saveMachineRequest = async (shopId: string, data: {
  plan_name: string; full_name: string; phone: string; address: string; city: string; state: string; zip_code: string; logo_url: string; needs_logo_design: boolean; notes: string;
}) => {
  const { error } = await supabase.from('machine_requests').upsert({
    shop_id: shopId, status: 'filled', ...data, updated_at: new Date().toISOString()
  }, { onConflict: 'shop_id' });
  if (error) throw error;
};

export const checkMachineRequestExists = async (shopId: string): Promise<boolean> => {
  const { data } = await supabase.from('machine_requests').select('id').eq('shop_id', shopId).maybeSingle();
  return !!data;
};